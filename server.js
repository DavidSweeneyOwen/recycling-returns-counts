...
const server = http.createServer((req, res) => {
  ...
  /* Collections still awaiting a count 3 months after the SO was raised drop off
     Awaiting Count into the Archive tab on age alone. This pulls one back when the
     office knows it is genuinely still coming. */
  if (req.method === 'POST' && p === '/api/keep-open') {
    return readBody(req, body => {
      try {
  ...
  if (req.method === 'POST' && p === '/api/set-so') {
    return readBody(req, body => {
      try {
        ...
        o.dropRef = o.dropRef || o.so;     // keep the DROP- reference for the audit trail
        o.so = norm;

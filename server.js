...
const server = http.createServer((req, res) => {
  ...
  const OPEN_PATHS = ['/count', '/api/ping', '/api/counter-config', '/api/find-order', '/api/count', '/api/manual-order'];
  ...
  if (req.method === 'POST' && p === '/api/manual-order') {
    return readBody(req, body => {
      try {
        const { cust, crates, by } = JSON.parse(body);
        const custN = String(cust || '').trim();
        const n = Math.max(1, parseInt(crates, 10) || 0);
        if (!custN) return json(res, 200, { error: 'Enter the customer name' });
        if (!n) return json(res, 200, { error: 'Enter how many cages / pallet boxes are in this delivery' });
        const so = 'MANUAL-' + (++db.seq);
        const o = {
          id: db.seq, so, cust: custN, date: new Date().toISOString().slice(0, 10),
          by: String(by || '').trim(), crates: n, counts: [], status: 'open',
          wtn: null, finalSo: null, source: 'manual'
        };
        db.orders.unshift(o);
        saveDb();
  ...
  if (req.method === 'POST' && p === '/api/set-so') {
    return readBody(req, body => {
      try {
        const { orderId, so } = JSON.parse(body);
        const o = db.orders.find(x => x.id === Number(orderId));
        if (!o) return json(res, 404, { error: 'Order not found' });
